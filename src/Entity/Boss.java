...
public class Boss extends Monster{
	...
	public void deplacer(){
		...
		if(inMotion <= 0){
			...
			// on fait un test aleatoire pour savoir si il va commencer un deplacement
			double testDepla = actualRandRange * Math.random();
			
			// si oui
			if(testDepla < 1.0){
				
				// on reinitialise le test
				actualRandRange = maxRandRange;
				
				// on determine ou on va
				int indiceSpot = randG.nextInt(spotCoordDs.size());
				
				//on calcule le vercteur de deplacement pour y allez
				double longDeplaTT = spotCoordDs.get(indiceSpot).x - (x + (image.getWidth() / 2) / Jeu.Jeu.resolution.getWidth());	// oriente
				double hautDeplaTT = spotCoordDs.get(indiceSpot).y - (y + (image.getHeight() / 2) / Jeu.Jeu.resolution.getHeight());	// oriente
				...
				// on determine le nombre de deplacement a faire pour rejoindre le spot
				inMotion = (int)(hypo / moveSpeed);
			}
			//sinon
			else{
				// on augmente la probabilite de reussir le test
				actualRandRange *= 0.99;
			}
		}
		// sinon il poursuit juste son deplacement
		else{
			inMotion --;
